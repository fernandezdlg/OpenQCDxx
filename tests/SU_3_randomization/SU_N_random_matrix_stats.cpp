...
int main() {
    ...
    for (size_t i = 0; i < N_samples; ++i) {
        ...
        Eigen::MatrixXcd Q2 = Q * L;

        // Set to SU(3)
        Q /= std::pow(Q.determinant(), 1.0/N);
        Q2 /= std::pow(Q2.determinant(), 1.0/N);
    ...
    std::ofstream file;
    file.open("sun_eigenvalues_HH.csv");
    file << "modulus,phase\n";
    ...
    file.open("sun_eigenvalues_HH_corr.csv");
    file << "modulus,phase\n";
    ...
    // Save the eigenvalues to a csv file with 12 decimal places
    file.open("sun_eigenvalues_colpiv.csv");
    file << "modulus,phase\n";
    for (size_t i = 0; i < eigenvalues_modulus.size(); ++i) {
        file << std::fixed << std::setprecision(12) << eigenvalues_modulus[i] << "," << eigenvalues_phase[i] << "\n";
    }
    ...
    file.open("sun_eigenvalues_colpiv_corr.csv");
    file << "modulus,phase\n";
    for (size_t i = 0; i < eigenvalues_modulus2.size(); ++i) {
        file << std::fixed << std::setprecision(12) << eigenvalues_modulus2[i] << "," << eigenvalues_phase2[i] << "\n";
    }
    ...
    // Save the eigenvalues to a csv file with 12 decimal places
    file.open("sun_eigenvalues_fullpiv.csv");
    file << "modulus,phase\n";
    for (size_t i = 0; i < eigenvalues_modulus.size(); ++i) {
        file << std::fixed << std::setprecision(12) << eigenvalues_modulus[i] << "," << eigenvalues_phase[i] << "\n";
    }
    ...
    // Save the eigenvalues to a csv file with 12 decimal places
    file.open("sun_eigenvalues_fullpiv_corr.csv");
    file << "modulus,phase\n";
    for (size_t i = 0; i < eigenvalues_modulus2.size(); ++i) {
        file << std::fixed << std::setprecision(12) << eigenvalues_modulus2[i] << "," << eigenvalues_phase2[i] << "\n";
    }
